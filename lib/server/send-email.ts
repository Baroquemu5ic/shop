...
export async function sendEmail(payload : sendEmailProps) {
  try {
    ...
    if (resendMailId) {
      console.log(`NextJS-Server: sendEmail: RESEND success: ID: ${resendMailId}`)
      ...
      if (dbResult?.success === true) {
        console.log(`NextJS-Server:sendEmail: SUPABASE success ID: ${resendMailId}`)
      } else if (dbResult?.error === true) {
        console.log(`NextJS-Server:sendEmail: SUPABASE failure ID: ${resendMailId}`)
      }
